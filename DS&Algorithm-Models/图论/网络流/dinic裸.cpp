...
int dis[MAXINT],n,r=0,m;   //n indicate the number of nodes in the graph, of which 1 is the source and n is the sink.
...
int dinic(int p,int flow) {
	...
	for(int i=0; i<tab[p].size(); i++) {
		edge &e = side[tab[p][i]];
		if(dis[e.t]==dis[p]+1&&e.v>0&&(a=dinic(e.t,min(flow,e.v)))) {
			e.v-=a;
			side[tab[p][i]^1].v+=a;
			return a;
		}
